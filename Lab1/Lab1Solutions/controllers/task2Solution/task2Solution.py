...
while robot.step(timestep) != -1:
    ...
    # Calculate Time
    T1 = calcTimeOfMotion('circle',[V,R1])
    T2 = calcTimeOfMotion('circle',[V,R2])
    print("Robot will take " + str(T1+T2) + " sec")
    start_time = robot.getTime()

    # Preforms the Rectangle Motion
    circleR(robot,R1,V,direction='left')
    circleR(robot,-R2,V,direction='right')
